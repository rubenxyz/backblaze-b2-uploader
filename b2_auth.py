...
class B2Auth:
    ...
    def get_1password_credentials(self) -> Dict[str, str]:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    Config.OP_CLI, "item", "get", Config.OP_ITEM_NAME,
                    "--format", "json"
                ],
                capture_output=True,
                text=True,
            ...
            if result.returncode != 0:
                logger.error(f"Failed to retrieve item '{Config.OP_ITEM_NAME}' from 1Password")
                logger.error(f"Error: {result.stderr}")
                raise B2AuthError(f"1Password item '{Config.OP_ITEM_NAME}' not found")
