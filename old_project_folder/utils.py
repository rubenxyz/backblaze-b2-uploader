...
def generate_link_file(output_dir: Path, files_processed: List[Dict[str, str]], bucket_name: str) -> Path:
    """Generate individual link files for each uploaded file with actual download URLs from B2."""
    timestamp = datetime.now()
    
    # Create a subdirectory for link files if needed
    ...
    if urls:
        ...
        for url in urls:
            ...
            if filename_match:
                ...
                link_file_path = links_dir / link_filename
    ...
    logger.info(f"Generated {files_created} individual link files in: {links_dir}")
    return links_dir
